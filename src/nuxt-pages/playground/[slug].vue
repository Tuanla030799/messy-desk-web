<template>
  <main>
    <section v-if="experiment" class="mx-auto max-w-6xl px-5 py-12">
      <NuxtLink to="/playground" class="sketch-link mb-8">Back to playground</NuxtLink>
      <div class="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div class="doodle-card min-h-[420px] overflow-hidden bg-white p-5">
          ...
          <div class="grid min-h-80 place-items-center rounded-[1.5rem] border-2 border-dashed border-ink bg-paper p-8">
            <DoodleLoader v-if="experiment.slug === 'doodle-loader'" label="loading idea" />
            <EmptySketch v-else-if="experiment.slug === 'empty-state-maker'" title="No ideas yet" description="The page is ready. Drop the first sketch here." />
    ...
    <section v-else class="mx-auto max-w-4xl px-5 py-12">
      <EmptySketch title="Experiment not found" description="This tiny lab may still be a sketch." />
    </section>
  </main>
</template>
