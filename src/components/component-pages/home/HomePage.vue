<template>
  <LayoutPageMain>
    <LayoutPageSection :spacious="false" class="grid min-h-[calc(100vh-82px)] items-center gap-10 py-10 lg:grid-cols-[1.05fr_0.95fr]">
      <div>
        ...
        <div class="doodle-card absolute inset-x-0 bottom-3 mx-auto max-w-lg overflow-hidden bg-white p-5">
          ...
          <div class="rounded-[1.5rem] border-2 border-ink bg-paper p-5">
            ...
            <div class="mt-5 rounded-[1.5rem] border-2 border-dashed border-ink bg-white p-4">
              <FeedbackDoodleLoader label="sketching" />
            </div>
          </div>
        </div>
      </div>
    ...
    <LayoutPageSection>
      <LayoutSectionHeader eyebrow="Fresh scraps" title="Latest from the desk" to="/blog" action="See all posts" />
    ...
    <LayoutPageSection class="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <FeedbackEmptySketch title="Nothing here can stay boring" description="Empty states, loading moments va trang rong deu co quyen duoc minh hoa mot cach vui ve." />
